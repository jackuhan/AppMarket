...
public class DownloadManager {
   ...
   * @throws DbException
   */
  public synchronized void addNewDownload(String url, String fileName, String target, boolean autoResume, boolean autoRename, String icon,
      String ver, int myId, final RequestCallBack<File> callback, final DownloadInfo downloadInfo) throws DbException {
    downloadInfo.setDownloadUrl(url);
    downloadInfo.setAutoRename(autoRename);
    downloadInfo.setAutoResume(autoResume);
    downloadInfo.setFileName(fileName);
    downloadInfo.setFileSavePath(target);
    downloadInfo.setimage(icon);
    downloadInfo.setversion(ver);
    downloadInfo.setId(myId);
    ...
    Log.v("addNewDownload", downloadInfo.toString());
  ...
  private PendingIntent getPendingIntent(boolean isDownloadFinish, DownloadInfo downloadInfo) {
    ...
    if (isDownloadFinish) {//下载完成打开安装界面
      ...
      intent = new Intent();
      intent.setClass(mContext, com.szcx.market.activitys.DownloadList.class);
    }
  ...
  private void showCustomProgressNotify(DownloadInfo downloadInfo) {
    ...
    mRemoteViews.setImageViewResource(R.id.iv_image, R.drawable.ic_fenlei);
  ...
  public class ManagerCallBack extends RequestCallBack<File> {
    ...
    @Override public Object getUserTag() {
      if (baseCallBack == null) return null;
      return baseCallBack.getUserTag();
    }

    @Override public void setUserTag(Object userTag) {
    ...
    @Override public synchronized void onLoading(long total, long current, boolean isUploading) {
      HttpHandler<File> handler = downloadInfo.getHandler();
      if (handler != null) {
        downloadInfo.setState(handler.getState());
      }
  ...
  private class HttpHandlerStateConverter implements ColumnConverter<HttpHandler.State> {
    ...
    @Override public HttpHandler.State getFieldValue(Cursor cursor, int index) {
      return HttpHandler.State.valueOf(cursor.getInt(index));
    }

    @Override public HttpHandler.State getFieldValue(String fieldStringValue) {
      if (fieldStringValue == null) return null;
      return HttpHandler.State.valueOf(fieldStringValue);
    }

    @Override public Object fieldValue2ColumnValue(HttpHandler.State fieldValue) {
      return fieldValue.value();
    }

    @Override public ColumnDbType getColumnDbType() {
